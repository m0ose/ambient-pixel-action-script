package structuredlight
{
	...
	public class ProjectorMap extends EventDispatcher
	{
		...
		public function interpolate( autoCleanUp:Boolean = false, percentToKeep:Number = 1.0 ):void
		{
			...
			//interpolate
			interpolatedReverseAverage( percentToKeep );
		...
		public function interpolatedReverseAverage( percentToKeep:Number = 1.0):void
		{
			//
		...
		//
		public function interpolatedReverse( percentToKeep:Number = 1.0):void
		{
			...
			for( var x:int = 0 ; x < rev_map.width(); x++)
			{
				...
				for( var y:int=0; y< rev_map.height(); y++)
				{
					...
					{ 
						...
						var median_point:Point = rev_map.rev_map[x][y][Math.floor((rev_map.rev_map[x][y].length / 2))].clone()
						if( median_point.x > 0   &&   median_point.y > 0 )
						{
							var rnd:Number = Math.random() ;
							if( rnd < percentToKeep)
							{
								mypoints.push( new Point(x,y) )						
						  		proj_map[x][y] = median_point.clone()
							}
						}
